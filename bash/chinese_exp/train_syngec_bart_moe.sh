...
SEED=2022
FAIRSEQ_CLI_PATH=../../src/src_syngec/fairseq-0.10.2/fairseq_cli
MODEL_DIR_STAGE1=../../model/syngec_chinese_bart_moe/
PROCESSED_DIR_STAGE1=../../preprocess/chinese_hsk+lang8_with_syntax_transformer
FAIRSEQ_PATH=../../src/src_syngec/fairseq-0.10.2/fairseq
...
cp ./train_syngec_bart_moe.sh $MODEL_DIR_STAGE1

# Enhanced MoE Training with Differential Learning Rates
...
CUDA_VISIBLE_DEVICES=0 nohup python -u $FAIRSEQ_CLI_PATH/train.py $PROCESSED_DIR_STAGE1/bin \
    ...
    --only-gnn \
    --syntax-encoder GCN \
    --use-moe-decoder True \
    --moe-num-experts 4 \
    --moe-gate SwitchGate \
    --top-k 2 \
    --freeze-bart-parameters \
    --freeze-encoder-parameters \
    --moe-gate-lr-scale 3.0 \
    --expert-lr-scale 1.0 \
    --load-balancing-loss-weight 0.01 \
    --expert-dropout 0.1 \
    --moe-gate-warmup-epochs 5 \
